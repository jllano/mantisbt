...
function require_mantis_core() {
	$t_var_list = array();
	parse_config_global_vars( dirname( dirname( dirname( __FILE__ ) ) ) . 'config_defaults_inc.php', $t_var_list );
	parse_config_global_vars( dirname( dirname( dirname( __FILE__ ) ) ) . 'config/config_inc.php', $t_var_list );
...
# distribution.
$g_mantisRoot = dirname( dirname( dirname( __FILE__ ) ) );
...
$g_mantisHelpdeskTests = $g_mantisRoot . '/plugins/Helpdesk/tests';
...
$g_path = array(
	...
	$g_mantisLibrary,
	$g_mantisClasses,
	$g_mantisHelpdeskTests,
	get_include_path()
);
set_include_path( implode( PATH_SEPARATOR, $g_path ) );

# Unset global variables that are no longer needed.
unset($g_mantisRoot, $g_mantisLibrary, $g_mantisTests, $g_mantisHelpdeskTests, $g_path);
