...
function xmlhttprequest_plugin_zendesk_articles() {
	...
	$url = 'http://' . $t_subdomain . '.zendesk.com/api/v2/help_center/articles/search.json?query=' . urlencode( $query ) . '&sort_by=title';
	...
	$t_token = plugin_config_get( 'token' );

	if ( !is_blank( $t_user ) && !is_blank( $t_token ) ) {
		$t_basic_auth = $t_user . '/token:' . $t_token;
